...
public class EnumCloneGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(compilationAndMaps, (spc, pair) =>
        {
            ...
            foreach (var (targetFullName, sourceFullName) in maps)
            {
                ...
                var sourceSymbol = GetTypeByMetadataNameExtended(compilation, sourceFullName);
                ...
                var hasFlags = sourceNamed.GetAttributes()
                ...
                var flagsPrefix = hasFlags ? "[Flags]" + NewLine : string.Empty;
    ...
    /// <summary> Fetch an enum even from a nested type by successively trying to replace the last '.' delimiter with a '+'. </summary>
    /// <returns> The first found matching type or null. </returns>
    private INamedTypeSymbol? GetTypeByMetadataNameExtended(Compilation compilation, string name)
    {
        ...
        while (dot >= 0)
        {
            sb[dot] = '+';
            name = sb.ToString();
            if (compilation.GetTypeByMetadataName(name) is { } nestedType)
                return nestedType;
